...
void cubic_traj_api(const mxArray *const prhs[5], const mxArray **plhs)
{
  emlrtStack st{
      nullptr, // site
      nullptr, // tls
  ...
  CT = (real_T(*)[12])mxMalloc(sizeof(real_T[12]));
  ...
  pi = b_emlrt_marshallIn(st, emlrtAlias(prhs[3]), "pi");
  pf = b_emlrt_marshallIn(st, emlrtAlias(prhs[4]), "pf");
  // Invoke the target function
  cubic_traj(tf, *vi, *vf, *pi, *pf, *CT);
  // Marshall function outputs
  *plhs = emlrt_marshallOut(*CT);
}

void cubic_traj_atexit()
{
  emlrtStack st{
      nullptr, // site
      nullptr, // tls
  ...
  cubic_traj_xil_terminate();
  cubic_traj_xil_shutdown();
  emlrtExitTimeCleanup(&emlrtContextGlobal);
}

void cubic_traj_initialize()
{
  emlrtStack st{
      nullptr, // site
      nullptr, // tls
...
// End of code generation (_coder_cubic_traj_api.cpp)
